...
function income_preprocess_html(&$variables) {
  ...
  if (!empty($disable_switcher) && $disable_switcher == 'on') {
	drupal_add_css(base_path() . path_to_theme() . '/js/style-switcher/color-switcher.css', array(
	  'type' => 'external',
	  'media' => 'all'
	));
  }
...
function getRelatedPosts($ntype, $nid) {
  ...
  $nids = db_query("SELECT n.nid, title FROM {node} n WHERE n.status = 1 AND n.type = :type AND n.nid <> :nid ORDER BY RAND() LIMIT 0,6", array(
	':type' => $ntype,
	':nid' => $nid
  ))->fetchCol();
  ...
  if (!empty($nodes)):
	...
	foreach ($nodes as $node) :
	  ...
	  $return_string .= theme('image_style', array(
		'style_name' => 'image_112x70',
		'path' => $field_image[0]['uri'],
		'attributes' => array('alt' => $node->title)
	  ));
